...
impl<'a> PdfAttachments<'a> {
    ...
    pub fn len(&self) -> PdfAttachmentIndex {
        self.bindings().FPDFDoc_GetAttachmentCount(self.document_handle) as PdfAttachmentIndex
    }
    ...
    pub fn create_attachment_from_bytes(&mut self, name: &str, bytes: &[u8]) -> Result<PdfAttachment<'_>, PdfiumError> {
        ...
        let handle = self.bindings().FPDFDoc_AddAttachment_str(self.document_handle, name);
