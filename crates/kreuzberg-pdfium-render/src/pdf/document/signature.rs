...
impl PdfSignatureModificationDetectionPermission {
    ...
    pub(crate) fn from_pdfium(raw: c_uint) -> Result<Self, PdfiumError> {
        match raw {
            0 => Err(PdfiumError::PdfiumLibraryInternalError(PdfiumInternalError::Unknown)),
...
impl<'a> PdfSignature<'a> {
    #[inline]
    pub(crate) fn from_pdfium(handle: FPDF_SIGNATURE, bindings: &'a dyn PdfiumLibraryBindings) -> Self {
    ...
    pub fn bytes(&self) -> Vec<u8> {
        ...
        let result =
            self.bindings
                .FPDFSignatureObj_GetContents(self.handle, buffer.as_mut_ptr() as *mut c_void, buffer_length);
    ...
    pub fn reason(&self) -> Option<String> {
        ...
        let result =
            self.bindings
                .FPDFSignatureObj_GetReason(self.handle, buffer.as_mut_ptr() as *mut c_void, buffer_length);
    ...
    pub fn signing_date(&self) -> Option<String> {
        ...
        let result =
            self.bindings
                .FPDFSignatureObj_GetTime(self.handle, buffer.as_mut_ptr() as *mut c_char, buffer_length);
    ...
    pub fn modification_detection_permission(
        ...
        PdfSignatureModificationDetectionPermission::from_pdfium(
            self.bindings.FPDFSignatureObj_GetDocMDPPermission(self.handle),
        )
    }
}
