...
impl<'a> PdfBookmarks<'a> {
    ...
    pub fn find_first_by_title(&self, title: &str) -> Result<PdfBookmark<'_>, PdfiumError> {
        let handle = self.bindings.FPDFBookmark_Find_str(self.document_handle, title);
    ...
    pub fn iter(&self) -> PdfBookmarksIterator<'_> {
        PdfBookmarksIterator::new(self.root(), true, None, self.document_handle(), self.bindings())
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while let Some((node, parent)) = self.pending_stack.pop() {
            ...
            self.pending_stack.push((
                self.bindings.FPDFBookmark_GetNextSibling(self.document_handle, node),
                parent,
            ));
            ...
            if self.include_descendants {
                self.pending_stack.push((
                    self.bindings.FPDFBookmark_GetFirstChild(self.document_handle, node),
                    node,
                ));
            }
