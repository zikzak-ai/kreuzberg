...
impl<'a> PdfAttachment<'a> {
    #[inline]
    pub(crate) fn from_pdfium(handle: FPDF_ATTACHMENT, bindings: &'a dyn PdfiumLibraryBindings) -> Self {
    ...
    pub fn name(&self) -> String {
        ...
        let result =
            self.bindings()
                .FPDFAttachment_GetName(self.handle, buffer.as_mut_ptr() as *mut FPDF_WCHAR, buffer_length);
    ...
    pub fn len(&self) -> usize {
        ...
        if self.bindings().is_true(self.bindings().FPDFAttachment_GetFile(
            self.handle,
            std::ptr::null_mut(),
            0,
            &mut out_buflen,
    ...
    pub fn save_to_writer<W: Write>(&self, writer: &mut W) -> Result<(), PdfiumError> {
        self.save_to_bytes()
            .and_then(|bytes| writer.write_all(bytes.as_slice()).map_err(PdfiumError::IoError))
    }
