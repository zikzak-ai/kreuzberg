...
def test_config_chunking_heading_context() -> None:
    ...
    assert result.chunks_have_content is True
    assert result.chunks_have_heading_context is True
...
def test_config_chunking_prepend_heading_context() -> None:
    ...
    assert result.chunks_have_content is True
    assert result.chunks_have_heading_context is True
    assert result.first_chunk_starts_with_heading is True
...
def test_config_document_structure_groups() -> None:
    ...
    result = extract_file_sync(path=path, config=config)
    assert any(
        v in result.mime_type for v in ["application/vnd.openxmlformats-officedocument.wordprocessingml.document"]
    )
    assert result.document_structure
    assert result.document_structure_has_groups is True
...
def test_config_html_styled_custom_css() -> None:
    ...
    path = "pdf/fake_memo.pdf"
    config = ExtractionConfig(
        html_output={"css": ".kb-p { color: red; }", "embed_css": True, "theme": "unstyled"}, output_format="html"
    )
    result = extract_file_sync(path=path, config=config)
...
def test_config_llm_embeddings() -> None:
    ...
    config = ExtractionConfig(
        chunking={
            "embedding": {
                "model": {"llm": {"model": "openai/text-embedding-3-small"}, "type": "llm"},
                "normalize": True,
            },
            "max_chars": 500,
            "max_overlap": 50,
...
def test_config_llm_structured_extraction() -> None:
    ...
    config = ExtractionConfig(
        structured_extraction={
            ...
            "schema": {
                "properties": {"date": {"type": "string"}, "summary": {"type": "string"}, "title": {"type": "string"}},
                "required": ["title"],
                "type": "object",
            },
