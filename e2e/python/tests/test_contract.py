...
# alef:hash:dcd56efad99b1da7f1abf1dbe03f982920962315aa9629f57e33e5ac8b2d033e
# To regenerate: alef generate
# To verify freshness: alef verify --exit-code
# Issues & docs: https://github.com/kreuzberg-dev/alef

"""E2e tests for category: contract."""from pathlib import Pathimport pytest  # noqa: F401from kreuzberg import extract_file, extract_file_sync, extract_bytes_sync, ExtractionConfig
...
@pytest.mark.asyncio
def test_api_batch_bytes_async() -> None:
    """Tests async batch bytes extraction API (batch_extract_bytes)."""    path = "pdf/fake_memo.pdf"
    config = None
...
@pytest.mark.skip(reason="WASM uses synchronous-only API; async extraction is not available on the wasm target")
@pytest.mark.asyncio
def test_api_batch_bytes_with_configs_async() -> None:
    """Tests async batch bytes extraction with per-file configs (batch_extract_bytes with file_configs parameter)."""    path = "pdf/fake_memo.pdf"
...
@pytest.mark.skip(reason="WASM uses synchronous-only API; async extraction is not available on the wasm target")
@pytest.mark.asyncio
def test_api_batch_file_async() -> None:
    """Tests async batch file extraction API (batch_extract_file)."""    path = "pdf/fake_memo.pdf"
    config = None
    result = await extract_file(path=path, config=config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
...
@pytest.mark.skip(reason="WASM uses synchronous-only API; async extraction is not available on the wasm target")
@pytest.mark.asyncio
def test_api_batch_file_with_configs_async() -> None:
    """Tests async batch file extraction with per-file configs (batch_extract_files with file_configs parameter)."""    path = "pdf/fake_memo.pdf"
    config = ExtractionConfig(output_format="markdown")
    result = await extract_file(path=path, config=config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
...
@pytest.mark.skip(reason="WASM uses synchronous-only API; async extraction is not available on the wasm target")
@pytest.mark.asyncio
def test_api_extract_bytes_async() -> None:
    """Tests async bytes extraction API (extract_bytes)."""    path = "pdf/fake_memo.pdf"
    config = None
    result = await extract_file(path=path, config=config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
...
@pytest.mark.skip(reason="WASM uses synchronous-only API; async extraction is not available on the wasm target")
@pytest.mark.asyncio
def test_api_extract_file_async() -> None:
    """Tests async file extraction API (extract_file)."""    path = "pdf/fake_memo.pdf"
    config = None
    result = await extract_file(path=path, config=config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
...
def test_config_chunking_prepend_heading_context() -> None:
    """Tests markdown chunker prepends heading hierarchy to chunk content."""    path = "markdown/extraction_test.md"
...
def test_config_document_structure_with_headings() -> None:
    """Tests document structure with DOCX heading-driven nesting."""    path = "docx/fake.docx"
...
def test_config_element_types() -> None:
    """Tests element-based result format with element type assertions on DOCX."""    path = "docx/unit_test_headers.docx"
...
def test_config_embedding_plugin() -> None:
    """Tests EmbeddingModelType::Plugin variant deserialization in ChunkingConfig — config accepts the plugin variant shape; actual dispatch requires a host-language backend registered via register_embedding_backend at runtime."""    path = "pdf/fake_memo.pdf"
...
def test_config_extraction_timeout() -> None:
    """Tests that extraction_timeout_secs config field is accepted and does not affect fast extractions."""    path = "pdf/fake_memo.pdf"
...
def test_config_keywords() -> None:
    """Tests keyword extraction via YAKE algorithm."""    path = "pdf/fake_memo.pdf"
...
def test_config_llm_structured_extraction() -> None:
    """Tests structured extraction via liter-llm with JSON schema."""    path = "pdf/fake_memo.pdf"
...
def test_config_pages() -> None:
    """Tests page extraction and page marker configuration."""    path = "pdf/fake_memo.pdf"
...
def test_config_quality_enabled() -> None:
    """Tests quality scoring produces a score value in [0.0, 1.0]."""    path = "pdf/fake_memo.pdf"
...
def test_config_security_limits() -> None:
    """Tests archive extraction with custom security limits."""    path = "archives/documents.zip"
...
def test_config_tree_sitter() -> None:
    """Tests tree-sitter configuration round-trip."""    path = "code/hello.py"
...
def test_output_format_bytes_markdown() -> None:
    """Tests markdown output format via bytes extraction API."""    data = Path("pdf/fake_memo.pdf").read_bytes()
...
def test_output_format_markdown() -> None:
    """Tests Markdown output format."""    path = "pdf/fake_memo.pdf"
    config = ExtractionConfig(output_format="markdown")
    result = extract_file_sync(path=path, config=config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
