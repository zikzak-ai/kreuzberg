...
# alef:hash:51298c5f6bed1a7d07d0cc2e36a7a1c061e521a4bfd78037ec4c82730377d3f0
...
test_that("format_pptx: PPTX presentation extraction using extract_file_sync", {
  result <- jsonlite::fromJSON(extract_file_sync(path = .resolve_fixture("pptx/simple.pptx"), mime_type = "application/vnd.openxmlformats-officedocument.presentationml.presentation"), simplifyVector = FALSE)
  expect_true(TRUE)
})

test_that("format_xlsx: XLSX spreadsheet extraction using extract_file_sync", {
  result <- jsonlite::fromJSON(extract_file_sync(path = .resolve_fixture("xlsx/stanley_cups.xlsx"), mime_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"), simplifyVector = FALSE)
  expect_true(TRUE)
})
