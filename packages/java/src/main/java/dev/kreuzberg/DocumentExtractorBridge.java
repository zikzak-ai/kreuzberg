...
public final class DocumentExtractorBridge implements AutoCloseable {
    ...
    private int handleExtractBytes(MemorySegment userData, MemorySegment content_in, MemorySegment mime_type_in, MemorySegment config_in, MemorySegment outResult, MemorySegment outError) {
        try {
            ...
            InternalDocument result = impl.extract_bytes(content, mime_type, config);
    ...
    private int handleExtractFile(MemorySegment userData, MemorySegment path_in, MemorySegment mime_type_in, MemorySegment config_in, MemorySegment outResult, MemorySegment outError) {
        try {
            ...
            String config_json = config_in.reinterpret(Long.MAX_VALUE).getString(0);
            ExtractionConfig config = JSON.readValue(config_json, ExtractionConfig.class);
            InternalDocument result = impl.extract_file(path, mime_type, config);
            String json = JSON.writeValueAsString(result);
            MemorySegment jsonCs = arena.allocateFrom(json);
            outResult.set(ValueLayout.ADDRESS, 0, jsonCs);
            return 0;
    ...
    private int handleAsSyncExtractor(MemorySegment userData, MemorySegment outResult, MemorySegment outError) {
        try {
            SyncExtractor result = impl.as_sync_extractor();
            String json = JSON.writeValueAsString(result);
            MemorySegment jsonCs = arena.allocateFrom(json);
            outResult.set(ValueLayout.ADDRESS, 0, jsonCs);
            return 0;
