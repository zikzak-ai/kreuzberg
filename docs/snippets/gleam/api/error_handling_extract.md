...
pub fn main() {
  ...
  case kreuzberg.batch_extract_files_sync(items, default_config()) {
    Ok(results) ->
      io.println("Extracted " <> int.to_string(list.length(results)) <> " files")
    Error(kreuzberg.UnsupportedFormat(mime)) ->
      io.println_error("Format not supported: " <> mime)
    Error(kreuzberg.KreuzbergErrorOcr(message: message, source: _)) ->
      io.println_error("OCR failed: " <> message)
    Error(kreuzberg.MissingDependency(dep)) ->
      io.println_error("Missing dependency — install " <> dep)
    Error(_) -> io.println_error("Batch extraction failed")
  }
}
```
