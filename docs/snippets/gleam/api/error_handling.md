...
pub fn main() {
  case kreuzberg.extract_file_sync("document.pdf", option.None, default_config()) {
    ...
    Error(kreuzberg.Io(message)) ->
      io.println_error("File error: " <> message)
    Error(kreuzberg.UnsupportedFormat(mime)) ->
      io.println_error("Unsupported format: " <> mime)
    Error(kreuzberg.Parsing(message: message, source: _)) ->
      io.println_error("Corrupt or invalid document: " <> message)
