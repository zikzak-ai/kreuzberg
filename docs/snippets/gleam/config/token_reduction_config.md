...
// `TokenReductionOptions` is the Gleam analogue of Rust's
// `TokenReductionConfig` — same fields, language-native naming.
...
pub fn main() {
  case kreuzberg.extract_file_sync("document.pdf", option.None, config()) {
    Ok(result) -> io.println("Reduced content: " <> result.content)
