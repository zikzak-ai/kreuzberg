...
fn config() -> kreuzberg.ExtractionConfig {
  kreuzberg.ExtractionConfig(
    ...
    // `result_format` controls Unified vs ElementBased.
    result_format: kreuzberg.ElementBased,
    ...
    Ok(result) -> {
      case result.elements {
        option.Some(elements) ->
          io.println("Elements: " <> int.to_string(list.length(elements)))
        option.None -> io.println("No elements produced")
      }
    }
