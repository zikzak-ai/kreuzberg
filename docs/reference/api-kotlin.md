...
| `entries` | `List<Any>?` | `[]` | Raw BibTeX entry data (author, title, year, etc. per entry) |
...
| `customProperties` | `Map<String, Any>?` | `{}` | Custom office properties from docProps/custom.xml |
...
| `additional` | `Map<String, Any>` | `{}` | Custom fields for plugin-injected and format-specific dynamic data (e.g., OCR backend metadata, org-mode directives). Uses `Cow<'static, str>` keys so static string keys avoid allocation. |
...
| `customFields` | `Map<String, Any>?` | `{}` | Pass-through of custom fields not mapped to standard metadata |
