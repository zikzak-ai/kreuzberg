...
| `entries` | `List(String)?` | `[]` | Raw BibTeX entry data (author, title, year, etc. per entry) |
...
| `extraHeaders` | `Dict(String, String)?` | `{}` | Non-standard email headers as key-value pairs |
...
| `customProperties` | `Dict(String, String)?` | `{}` | Custom office properties from docProps/custom.xml |
...
| `additional` | `Dict(String, String)` | `{}` | Custom fields for plugin-injected and format-specific dynamic data (e.g., OCR backend metadata, org-mode directives). Uses `Cow<'static, str>` keys so static string keys avoid allocation. |
...
| `customFields` | `Dict(String, String)?` | `{}` | Pass-through of custom fields not mapped to standard metadata |
