...
| `entries` | `[ArchiveFileEntry]` | `[]` | Typed entries with path, size, and is_dir fields |
...
| `entries` | `[String]?` | `[]` | Raw BibTeX entry data (author, title, year, etc. per entry) |
...
| `extraHeaders` | `[String: String]?` | `{}` | Non-standard email headers as key-value pairs |
...
| `customProperties` | `[String: String]?` | `{}` | Custom office properties from docProps/custom.xml |
...
| `additional` | `[String: String]` | `{}` | Custom fields for plugin-injected and format-specific dynamic data (e.g., OCR backend metadata, org-mode directives). Uses `Cow<'static, str>` keys so static string keys avoid allocation. |
...
| `fieldCount` | `UInt64` | — | Number of top-level fields |
| `customFields` | `[String: String]?` | `{}` | Pass-through of custom fields not mapped to standard metadata |
